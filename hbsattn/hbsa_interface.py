...
class _HBSAttentionFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, cu_q_seqlens, cu_k_seqlens, block_mask, q_block_size, k_block_size, causal, softmax_scale, tile_mode = 'auto', num_block_per_group = 1, num_q_block = None, cu_q_block = None, q_block_to_batch = None, cu_num_q_block = None, num_k_block = None, cu_k_block = None, k_block_to_batch = None, cu_num_k_block = None, cu_num_q_group = None, q_group_to_batch = None):
        ...
        if callable(tile_mode): # the kernel will be lanunched with the grouping of q blocks determind by a scheduling algorithm and `num_block_per_group`
            ...
            if cu_num_q_group is None or q_group_to_batch is None:
                cu_num_q_group, q_group_to_batch = caculate_groups(cu_num_q_block, num_block_per_group)
            
            return _forward_scheduling(q, k, v, cu_q_seqlens, cu_k_seqlens, block_mask, q_block_size, k_block_size, causal, softmax_scale, tile_mode, num_block_per_group, num_q_block, cu_q_block, q_block_to_batch, cu_num_q_block, num_k_block, cu_k_block, k_block_to_batch, cu_num_k_block, cu_num_q_group, q_group_to_batch)
