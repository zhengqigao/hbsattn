...
def test_attention_configs(causal, nhead_q, nhead_k, softmax_scale, dtype, q_block_size, k_block_size, k_q_same_seqlen):
    ...
    assert torch.allclose(golden_ref_v1, golden_ref_v2, atol=1e-3, rtol=1e-3)
    assert torch.allclose(golden_ref_v1, golden_ref_v3, atol=1e-3, rtol=1e-3)
    assert torch.allclose(golden_ref_v1, out, atol=1e-3, rtol=1e-3)
