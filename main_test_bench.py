...
if __name__ == "__main__":
    ...
    cu_k_seqlens = torch.tensor([0,32, 61, 100], dtype=torch.int32, device=device) # [0, 32, 64, 96, 128, 160] # , 61, 100, 134, 157
    max_k_seqlen = int((cu_k_seqlens[1:] - cu_k_seqlens[:-1]).max().item())
    k_seqlen = cu_k_seqlens[-1].item()
    
    cu_q_seqlens = torch.tensor([0, 32, 64, 96], dtype=torch.int32, device=device) # [0, 32, 64, 96, 128, 160]
