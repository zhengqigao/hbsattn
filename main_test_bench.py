...
if __name__ == "__main__":
    ...
    q = torch.load("q.pt", map_location=device).to(dtype)
    k = torch.load("k.pt", map_location=device).to(dtype)
    v = torch.load("v.pt", map_location=device).to(dtype)
    block_mask = torch.load("block_mask.pt", map_location=device)
